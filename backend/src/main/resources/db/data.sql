...
-- 1. LIMPIEZA DE TABLAS (En orden correcto por las claves foráneas)
DELETE FROM materia_estado;
DELETE FROM correlatividad;
DELETE FROM materia;
DELETE FROM usuario;

-- Reseteamos los contadores de ID para que el usuario siempre sea el ID 1
...
VALUES ('admin@admin.com', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOcd7QA8qkrjm', 'Admin', 'ADMIN');
...
-- ELECTIVAS (Incluye Seminario Integrador)
INSERT INTO materia (id, nombre, anio, cuatrimestre, es_libre, es_configurable, es_electiva, creditos) VALUES
...
(123, 'Seminario Integrador', 3, 2, FALSE, FALSE, TRUE, 4); -- Agregada


-- 4. CARGA DE CORRELATIVIDADES (CURSAR)
...
-- Correlativas Seminario Integrador:
(123, 16, 'CURSAR'), (123, 6,  'CURSAR'), (123, 8,  'CURSAR'), 
(123, 13, 'CURSAR'), (123, 14, 'CURSAR');

-- 5. CARGA DE CORRELATIVIDADES (APROBAR)
INSERT INTO correlatividad (materia_id, correlativa_id, tipo) VALUES
...
-- 6. ESTADOS INICIALES (AHORA VINCULADOS AL USUARIO 1)
-- IMPORTANTE: Aquí está el cambio clave. Vinculamos el estado inicial al usuario ID 1
INSERT INTO materia_estado (usuario_id, materia_id, estado)
SELECT 1, id, 'PENDIENTE' FROM materia;